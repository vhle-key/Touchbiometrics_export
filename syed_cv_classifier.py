...
def eer_loss_bundled_function(y_true, y_pred_proba):
    ...
    # print("Shape", y_true.shape, y_true_bundled.shape, y_pred_proba.shape, y_pred_proba_bundled.shape)
...
def helper():
    ...
    switcher = {
        1: 'syed_horizontal.csv',
        2: 'syed_vertical.csv'
    }
    feature_names = ['phoneID', 'postureID', 'userID', 'xStart', 'xStop', 'yStart', 'yStop', 'strokeDuration',
                     'end2endDist',
                     'trajectoryLength', 'Ratio_TrjLen2EELen', 'avgVelo', 'direction', 'interstrokeTime',
                     'startPressure',
                     'stopPressure', 'midPressure', '20percVelo', '50percVelo', '80percVelo']

    print("----------EXPERIMENT: SYED DATASETS------------")
    runs = get_int("Number of runs:")
    while True:
        case = get_int("1 for Syed horizontal strokes, 2 for vertical strokes:")
        device = get_int("Choose device: 1 for T10, 2 for T7, 3 for S3:")
        posture = get_int("Choose posture: 1, 2, 3 for P1(lying flat), P2(held in portrait), P3(held in landscape)")
        if case not in [1, 2] or device not in [1, 2, 3] or posture not in [1, 2, 3]:
    ...
    file_name = switcher.get(case)
    dataset_train = pd.read_csv(file_name, names=feature_names)
    # Choose the dataset with specific device-posture combination
    dataset_train = dataset_train[dataset_train['phoneID'] == device][dataset_train['postureID'] == posture]

    return train_size, n0_bundle, dataset_train, file_name, runs, device, posture
...
if __name__ == "__main__":
    """Constants and initial feature vectors of all strokess"""
    t_start = time()
    n_fake_user = 10
    train_size, n0_bundle, dataset_train, file_name, runs, device, posture = helper()
    col_feats = ['midPressure', 'xStart', 'yStart', 'xStop', 'yStop', 'avgVelo', 'strokeDuration',
                 'trajectoryLength', 'end2endDist']
    ...
    print("-----------BENCHMARKING FOR DATASET %r, DEVICE %i, POSTURE P %i------------" % (file_name, device, posture))
